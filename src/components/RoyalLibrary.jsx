...
export default function RoyalLibrary() {
    ...
    const [datePopup, setDatePopup] = useState(null); // { date, sessions }
    const [selectedDate, setSelectedDate] = useState(null); // calendar 하이라이트용
    ...
    const handleDelete = async (act) => {
        ...
        try {
            ...
            if (datePopup) {
                setDatePopup(prev => ({
                    ...prev,
                    sessions: prev.sessions.filter(s => s.id !== act.id)
                }));
            }
        ...
        <div className="p-4 space-y-5 animate-book max-w-2xl mx-auto pb-24">
            ...
            <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-[#0a1f0c] to-[#102213] border border-[#32673b]/60 p-5">
                <div className="absolute inset-0 opacity-10"
                    style={{ backgroundImage: 'radial-gradient(circle at 80% 20%, #2bee4b 0%, transparent 60%)' }} />
            ...
            <div className="flex items-center justify-between px-1">
                <p className="text-[10px] font-black text-slate-500 dark:text-gray-500 uppercase tracking-widest flex items-center gap-1.5">
                    <span className="material-symbols-outlined text-xs text-[#2bee4b]">history</span>
                    전체 기록
                </p>
                <p className="text-[10px] text-slate-400 dark:text-gray-600">{sessions.length}개</p>
            </div>

            {/* 기록 카드 리스트 */}
            {sessions.length === 0 ? (
                <div className="bg-white dark:bg-[#1a331d] border border-stone-200 dark:border-[#32673b] rounded-3xl p-10 text-center">
                    <span className="material-symbols-outlined text-4xl text-slate-300 dark:text-gray-600">auto_stories</span>
                    <p className="text-slate-400 dark:text-gray-500 text-sm mt-3 font-bold">아직 기록이 없습니다.</p>
                    <p className="text-slate-300 dark:text-gray-600 text-xs mt-1">첫 번째 독서 기록을 남겨보세요!</p>
                </div>
            ) : (
                    ...
                    {sessions.map(act => {
                        ...
                        return (
                            <SessionCard
                                ...
                                dateStr={dateStr}
                                mins={mins}
                                onToggle={() => setExpandedId(isExpanded ? null : act.id)}
            ...
            {datePopup && (
                <DatePopup
                    datePopup={datePopup}
                    onClose={() => { setDatePopup(null); setSelectedDate(null); }}
                    onDelete={handleDelete}
                />
            )}
        </div>
    ...
    return (
        <div className={`bg-white dark:bg-[#1a331d] border rounded-2xl overflow-hidden transition-all duration-300 ${
            isExpanded
                ...
                : 'border-stone-200 dark:border-[#2a5530] hover:border-stone-300 dark:hover:border-[#2bee4b]/30'
        }`}>
            <div className="flex items-center gap-3 px-4 py-3 cursor-pointer" onClick={onToggle}>
                ...
                <div className="flex-1 min-w-0">
                    ...
                    {act.bookAuthor && (
                        <p className="text-[10px] text-slate-400 dark:text-gray-500 truncate">{act.bookAuthor}</p>
                    )}
            ...
            {isExpanded && (
                <div className="px-4 pb-4 border-t border-stone-100 dark:border-white/5 pt-3">
                    {act.pages > 0 && (
                        <p className="text-[10px] text-slate-400 dark:text-gray-500 mb-2">
                            📖 {act.pages}페이지 읽음
                        </p>
                    )}
                    {act.note && (
...
function DatePopup({ datePopup, onClose, onDelete }) {
    ...
    return (
        <div className="fixed inset-0 z-[500] flex items-center justify-center p-4" onClick={onClose}>
            ...
            {/* 중앙 모달 */}
            <div
                className="relative bg-white dark:bg-[#0d1f0f] rounded-[32px] shadow-2xl border border-stone-200 dark:border-[#2a5530] max-h-[80vh] max-w-md w-full flex flex-col"
                ...
                <div className="px-5 py-3 border-b border-stone-100 dark:border-white/5 shrink-0">
                    ...
                    <div className="flex gap-3 mt-2">
                        <div className="flex items-center gap-1 text-[10px] text-slate-500 dark:text-gray-400">
                            <span className="material-symbols-outlined text-xs text-[#2bee4b]">schedule</span>
                            총 {totalMins}분
                        </div>
                        <div className="flex items-center gap-1 text-[10px] text-slate-500 dark:text-gray-400">
                        ...
                        <div className="flex items-center gap-1 text-[10px] text-slate-500 dark:text-gray-400">
                            <span className="material-symbols-outlined text-xs text-[#2bee4b]">bookmark</span>
                            {sessions.length}회 기록
                        </div>
                    </div>
                </div>
                        ...
                        sessions.map(act => {
                            ...
                            return (
                                <SessionCard
                                    ...
                                    mins={mins}
                                    onToggle={() => setExpandedId(isExpanded ? null : act.id)}
                                    onDelete={async (a) => { await onDelete(a); }}
                                />
                            );
                        })
                    )}
